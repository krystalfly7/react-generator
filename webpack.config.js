...
var entry = glob.sync('./src/' + list + '/index.js').reduce(function (prev, curr) {
    const key = curr.slice(6, -9);
    const jskey = key.replace(/\//g, '.');
    const htmlkey = key.replace(/\//g, '_');
    const chunks = [jskey];
    htmlPlugins.push(new HtmlWebpackPlugin({
      template: `./html/index.html`,
      filename: `${htmlPath}${htmlkey}.html`,
      chunks,
    }));
    prev[jskey] = [path.resolve(__dirname, curr)];
    return prev;
}, {});
var plugins = [];
if (isProd) {
  ...
  htmlPlugins.push(new HtmlWebpackPlugin({
    template: `./html/router.html`,
    filename: `${htmlPath}router.html`,
    chunks: ['router'],
  }));
